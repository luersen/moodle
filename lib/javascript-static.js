    ...
    YUI().use('io', function(Y) {
        ...
        if (M.cfg.developerdebug) {
            cfg.on.failure = function(id, o, args) {
                alert("Error updating user preference '" + name + "' using ajax. Clicking this link will repeat the Ajax call that failed so you can see the error: " + url);
            }
        }
...
/**
 * Makes a best effort to connect back to Moodle to retrieve a user preference.
 *
 * @param String name the name of the setting to be retrieved.
 */
M.util.get_user_preference = function(name, callback) {
    YUI().use('io', function(Y) {
        var url = M.cfg.wwwroot + '/lib/ajax/getuserpref.php?sesskey=' +
                ...
                on: {
                    success: function(id, o, args) {
                        if(Y.lang.isFunction(callback)){
                            callback(o.responseText, id, o, args);
                        } else {
                            Y.log('Callbacks to get_user_preference must now be functions. Please update your code to pass in a function instead.',
                            'warn', 'M.util.get_user_preference')
                        }
                    }
                }
            };
        ...
        if (M.cfg.developerdebug) {
            cfg.on.failure = function(id, o, args) {
                alert("Error retrieving user preference '" + name + "' using ajax. Clicking this link will repeat the Ajax call that failed so you can see the error: " + url);
            }
        }
